...
import { enqueueTask } from '../lib/api';
  ...
  return (
    <footer className="composer-shell">
        ...
        <div className="attachment-strip" role="list" aria-label={`已添加 ${images.length} 张图片`}>
          {images.map((image) => (
            <div className="attachment" role="listitem" key={image.id}>
              ...
              <button type="button" onClick={() => setImages((current) => current.filter((item) => item.id !== image.id))} title={`移除 ${image.name}`} aria-label={`移除 ${image.name}`}>
                <X size={14} />
              </button>
            </div>
          ))}
      ...
      <div className="composer-row">
        ...
        <button className="send-button" type="button" onClick={() => void dispatch()} disabled={sending}>
          <Send size={17} />{sending ? '派单中' : '派单'}
        </button>
      </div>
