...
玉兔6 本机约定：Meowa 已接入为系统共享能力。CLI 与 key 的单一事实源是：

```bash
python3 /Users/yutu6/玉兔6工作区/shared/tools/meowa/meowart_api.py
```

`MEOWART_API_KEY` 只从本机统一保险库读取，不要复制到本 skill 目录。
...
```bash
python3 /Users/yutu6/玉兔6工作区/shared/tools/meowa/meowart_api.py skill-doc --task "<brief user request>"
```
