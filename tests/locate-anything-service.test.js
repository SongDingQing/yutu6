...
async function main() {
  try {
    ...
    result = await LocateAnything.locate({ image_path: '/tmp/peekaboo.png', query: 'search button' });
    ...
    const health = await requestHandler(server.handler, { method: 'GET', url: '/api/vision/locate/health' });
    ...
    const apiLocate = await requestHandler(server.handler, {
      ...
      body: { image_path: '/tmp/peekaboo.png', query: 'search button' },
    });
