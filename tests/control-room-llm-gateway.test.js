...
assert(html.includes('role="region" aria-label="控制室主区" aria-live="polite" aria-busy="true"'), 'main control room region must have live/busy semantics');
...
assert(html.includes('role="listitem" title="${esc(label)}" aria-label="${esc(label)}"'), 'monitoring rows must expose full row labels');
