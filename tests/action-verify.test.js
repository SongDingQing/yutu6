...
function runActionVerify(mode) {
  ...
  const baseRunner = (node, ctx, attempt) => {
    calls.push({ node: node.id, attempt, goal: ctx.goal, previous: ctx.previous_computer_use_action_verify || null });
    return { vars: { baseAttempt: attempt }, evidence: { type: 'mock', attempt } };
  };
  ...
  const ctx = { goal: 'Click the target button', acceptance: 'computer-use click smoke', projectId: '控制台' };
