...
- 完成状态只能填 `完成`、`部分`、`未完成`; done gate 通过只接受所有行都是 `完成`。
...
- 视觉/UI 类必须有可核 peekaboo 图片截图路径(.png/.jpg/.webp/.gif)和 opus-4.8/Codex 对照设计挑错证据;`failure.json`、截图失败标记和`自验收已归档`都不算截图完成证据。
